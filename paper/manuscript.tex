...
% ---------------------------------------------------------------------------
%  How Quantum Is the Advantage?  --  manuscript (arXiv / Computers & Security)
% ---------------------------------------------------------------------------
...
\usepackage{graphicx}
\graphicspath{{../}{./}}  % ../figures locally; ./figures in the arXiv bundle
...
\title{\textbf{How Quantum Is the Advantage?}\\[2pt]
\large A Fair, Calibration- and Noise-Aware Benchmark and Attribution Audit of\\
Quantum Machine Learning for Network Intrusion Detection}

\author{
  Syeda Anshrah Gillani\thanks{Corresponding author: \texttt{SyedaAnshrah16@gmail.com}}\\
  \small Hamdard University, Karachi, Pakistan
  \and
  Mirza Samad Ahmed Baig\\
  \small Fandaqah, Al Khobar, Saudi Arabia\\
  \small \texttt{MirzaSamadcontact@gmail.com}
}
\date{2026}
...
\medskip
\noindent\textbf{Keywords:} quantum machine learning; network intrusion detection; hybrid
quantum-classical neural networks; quantum support vector machine; NISQ; reproducible
benchmark; class imbalance; model calibration
\end{abstract}
...
\begin{tabularx}{\textwidth}{@{}L{2.6cm} L{2.4cm} X c c@{}}
\toprule
\textbf{Paper} & \textbf{Method} & \textbf{Dataset(s)} & \textbf{Honest\newline baseline?} & \textbf{Attri-\newline bution?}\\
\midrule
...
Bhatnagar et al.~\cite{bhatnagar2026mqe} & QSVM+QNN+RF & CICIDS, TON\_IoT & Yes\newline (cl.\ wins) & No\\
Bellante et al.~\cite{bellante2025pca} & FT PCA-QML & NIDS datasets & Yes & FT/PCA\newline only\\
\textbf{This work} & \textbf{VQC+QSVM\newline +controls} & \textbf{4 datasets,\newline official splits} & \textbf{Yes\newline (5 tuned)} & \textbf{Yes\newline (NISQ)}\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[
  font=\small,
  qb/.style={rounded corners=2pt,draw=qpurple,fill=lightq,thick,minimum width=3.3cm,minimum height=1.0cm,align=center},
  cb/.style={rounded corners=2pt,draw=qblue,fill=lightc,thick,minimum width=3.3cm,minimum height=1.0cm,align=center},
  res/.style={rounded corners=2pt,draw=qgreen,fill=qgreen!12,thick,minimum width=3.3cm,minimum height=1.0cm,align=center},
  cl/.style={rounded corners=2pt,draw=qblue,fill=qblue!10,thick,minimum width=3.3cm,minimum height=1.0cm,align=center},
  dec/.style={diamond,aspect=1.7,draw=qorange,thick,fill=qorange!15,align=center,inner sep=2pt},
  arr/.style={-{Stealth[length=2.2mm]},thick,qgray},
  ]
  \node[qb] (q) {Quantum model\\[1pt]\scriptsize hybrid VQC / QSVM};
  \node[cb,below=1.0cm of q] (c) {Matched classical control\\[1pt]\scriptsize same front-end $+$ capacity\\[1pt]\scriptsize $+$ regularisation sweep};
  \node[dec,right=1.8cm of $(q)!0.5!(c)$] (d) {Gap\\closes?};
  \node[res,right=2.4cm of d] (qu) {Residual is \textbf{quantum}\\[1pt]\scriptsize report with significance};
  \node[cl,below=1.0cm of qu] (cla) {Advantage is \textbf{classical}\\[1pt]\scriptsize preprocessing / regularisation};
  \draw[arr] (q.east) -| (d.north);
  \draw[arr] (c.east) -| (d.south);
  \draw[arr] (d.north) |- node[pos=0.75,above,font=\scriptsize]{no} (qu.west);
  \draw[arr] (d.south) |- node[pos=0.75,below,font=\scriptsize]{yes} (cla.west);
\end{tikzpicture}}
...
\smallskip
{\footnotesize Best classical model per cell --- full view: SVM (NSL-KDD), MLP (UNSW), RF (CICIDS, ToN-IoT);
same-budget: XGBoost (NSL-KDD), MLP (UNSW), RF (CICIDS, ToN-IoT).}
\end{table}
...
\textbf{Qubit budget.} The 4\,GB GPU caps width at $\sim$16 qubits (noiseless) / 12 (noisy, $(2^n)^2$
memory); the main configuration uses eight, so we cannot probe the larger-width regime.
...
released at \url{https://github.com/Mirza-Samad-Ahmed-Baig/quantum-ids-benchmark}.
